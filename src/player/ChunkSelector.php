...
final class ChunkSelector
{
        ...
        for ($subRadius = 0; $subRadius < $radius; $subRadius++) {
            ...
            for ($x = $subRadius; $x >= $minX; --$x) {
                for ($z = $lastZ; $z <= $x; ++$z) {
                    ...
                    yield from $this->yieldQuadrants($centerX, $centerZ, $x, $z, $subRadius);
                    
                    if ($x !== $z) {
                        yield from $this->yieldMirroredQuadrants($centerX, $centerZ, $x, $z, $subRadius);
                    }
                }
            }
        }
    ...
    /**
     * Yield quadrants for the given x, z coordinates.
     */
    private function yieldQuadrants(int $centerX, int $centerZ, int $x, int $z, int $subRadius): \Generator
    {
        // Top right quadrant
    ...
    /**
     * Yield mirrored quadrants for the given x, z coordinates.
     */
    private function yieldMirroredQuadrants(int $centerX, int $centerZ, int $x, int $z, int $subRadius): \Generator
    {
